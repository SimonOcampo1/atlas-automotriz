...
export default async function BrandModelsPage({
  ...
  const items = visibleModels.map((model) => {
    ...
    return {
      ...
      actionLabel: hasGenerations
        ? translate(locale, "action.viewGenerations")
        : undefined,
  ...
  return (
    <div className="min-h-screen bg-linear-to-b from-background via-background to-muted/30">
        ...
        <div className="mx-auto flex w-full max-w-6xl flex-col gap-4 px-4 py-6">
          ...
          >
            <Link href="/" className="flex items-center gap-2">
              ...
              {translate(locale, "general.backToAtlas")}
            </Link>
          </Button>
          <div className="relative flex flex-col items-center gap-3 text-center sm:items-start sm:text-left">
            ...
            <div className="space-y-2">
              <p className="text-xs uppercase tracking-[0.2em] text-muted-foreground">
                {translate(locale, "brand.modelsByBrand")}
              </p>
              <h1 className="text-3xl font-semibold tracking-tight">
                {brandData.name}
              </h1>
              <p className="text-sm text-muted-foreground">
                {translate(locale, "brand.selectModel")}
              </p>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <Badge variant="secondary">
                {translate(locale, "counts.models", { count: visibleModels.length })}
              </Badge>
            </div>
          </div>
        </div>
      ...
      <main className="mx-auto flex w-full max-w-6xl flex-col gap-8 px-4 py-10">
        ...
        <ModelCatalog
          items={items}
          emptyLabel={translate(locale, "brand.noModels")}
          locale={locale}
        />
      </main>
    </div>
